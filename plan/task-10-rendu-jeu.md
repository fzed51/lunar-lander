...
- `design/palette.ts` (`PALETTE`, `CouleurLem`) et `design/font.ts`
  (`dessineTexte`) (T1).
- `@lem/engine` : `Camera` (`versEcran`, `versEcranPixel`, `avecZoom` qui
  **refuse** un zoom non entier, `borne`, `suit`, `bornesVisibles`) et les
  primitives `drawPixel`, `fillRect`, `strokeRect`, `drawPolyline`, `withClip`,
  plus l'existant `withTransform` / `withAlpha` (T4). Il n'y a **pas** de
  `withCamera` : le contexte canvas reste à l'échelle 1.
...
1. Ajouter dans `constants.ts` :
   ...
   - `SEUILS_ZOOM = { vers2: 60, retour1: 80, vers4: 25, retour2: 35 }` (m
...
5. Créer `packages/game/src/screens/game.ts`, qui implémente `Ecran` (T5).
   - `entre(params)` : démarre une partie via `state.ts` (T9) avec le niveau
     reçu.
   ...
   - `transitionDemandee()` : `"fin"` quand `statut === "fini"`, avec le résultat
     de la partie.
...
- **Ordre de dessin** figé : ciel, étoiles, terrain, replis, drapeau, particules,
  ...
  relief.
- **Hystérésis** : un LEM qui flotte exactement à 60 m ne doit pas faire clignoter
...
- `zoomSuivant` : 1 à 200 m depuis 1 ; 2 à 50 m depuis 1 ; **encore 2** à 70 m
  depuis 2 (hystérésis) ; 1 à 90 m depuis 2 ; 4 à 20 m depuis 2 ; **encore 4** à
  30 m depuis 4 ; 2 à 40 m depuis 4. Aucune sortie hors de `ZOOMS`.
